...
class Message : public MessageTypeDecorator<MessageType::BASE_MESSAGE>
{
  ...
  protected:
    ...
    uint8_t* payload_handle()
    {
        return data_.data() + sizeof(header_);
    }
  ...
  public:
    static Ptr Create()
    {
        return Ptr(new Message());
    }

    static Ptr Create(const Message& other)
    {
        return Ptr(new Message(other));
    }
    ...
    const OculusMessageHeader& header() const
    {
        return header_;
    }
    ...
    uint16_t message_version() const
    {
        return header_.msgVersion;
    }
...
class PingWrapper1 : public PingWrapper
{
  ...
  public:
    ...
    virtual PingWrapper::Ptr copy() const
    {
        return Create(this->msg_->copy());
    }
    ...
    virtual uint16_t range_count() const
    {
        return this->metadata().nRanges;
    }

    virtual uint16_t bearing_count() const
    {
        return this->metadata().nBeams;
    }
    ...
    virtual uint32_t ping_data_size() const
    {
    ...
    {
        return this->metadata().fireMessage.flags & 0x4;
    }

    virtual uint8_t master_mode() const
    {
        return this->metadata().fireMessage.masterMode;
    }
    ...
    virtual uint32_t ping_index() const
    {
        return this->metadata().pingId;
    }

    virtual uint32_t ping_firing_date() const
    {
        return this->metadata().pingStartTime;
    }
    ...
    {
        return this->metadata().frequency;
    }

    virtual double speed_of_sound_used() const
    {
        return this->metadata().speeedOfSoundUsed;
    }

    virtual double range_resolution() const
    {
        return this->metadata().rangeResolution;
    }
...
class PingWrapper2 : public PingWrapper
{
  ...
  public:
    ...
    virtual uint32_t ping_firing_date() const
    {
        return (uint32_t)this->metadata().pingStartTime;
    }
...
class PingMessage : public MessageTypeDecorator<MessageType::PING_MESSAGE>
{
  ...
  public:
    ...
    Message::ConstPtr message() const
    {
        return pingData_->message();
    }

    const OculusMessageHeader& header() const
    {
        return pingData_->header();
    }

    const std::vector<uint8_t>& data() const
    {
        return pingData_->data();
    }

    const TimePoint& timestamp() const
    {
        return pingData_->timestamp();
    }

    uint16_t range_count() const
    {
        return pingData_->range_count();
    }

    uint16_t bearing_count() const
    {
        return pingData_->bearing_count();
    }

    const int16_t* bearing_data() const
    {
        return pingData_->bearing_data();
    }

    const uint8_t* ping_data() const
    {
        return pingData_->ping_data();
    }

    uint32_t step() const
    {
        return pingData_->step();
    }

    uint32_t ping_data_size() const
    {
        return pingData_->ping_data_size();
    }
    ...
    {
        return pingData_->has_gains();
    }

    uint8_t master_mode() const
    {
        return pingData_->master_mode();
    }

    uint8_t sample_size() const
    {
        return pingData_->sample_size();
    }

    uint32_t ping_index() const
    {
        return pingData_->ping_index();
    }

    uint32_t ping_firing_date() const
    {
        return pingData_->ping_firing_date();
    }

    double range() const
    {
        return pingData_->range();
    }

    double gain_percent() const
    {
        return pingData_->gain_percent();
    }

    double frequency() const
    {
        return pingData_->frequency();
    }

    double speed_of_sound_used() const
    {
        return pingData_->speed_of_sound_used();
    }

    double range_resolution() const
    {
        return pingData_->range_resolution();
    }

    double temperature() const
    {
        return pingData_->temperature();
    }
